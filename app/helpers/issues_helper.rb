...
module IssuesHelper
  ...
  def render_custom_fields_header(issue)
    s = "<div style=\"width:100%; display:block\" id=\"myheader\"> "
    if issue.status.name=="未启动"
      s << "<a href=\"javascript:switchtab(1)\"><span id=\"custab1\" class=\"tab2\">问题</span></a>\n"
      s << "<a href=\"javascript:switchtab(3)\"><span id=\"custab3\" class=\"tab1\">其他</span></a>\n"
    else
      s << "<a href=\"javascript:switchtab(1)\"><span id=\"custab1\" class=\"tab2\">问题</span></a>"
      s << "<a href=\"javascript:switchtab(2)\"><span id=\"custab2\" class=\"tab1\">Sir</span></a>"
      s << "<a href=\"javascript:switchtab(3)\"><span id=\"custab3\" class=\"tab1\">其他</span></a>"
  ...
  def render_custom_fields_content(issue)
    values = issue.visible_custom_field_values
    return if values.empty?
    ordered_values = []
    ...
    s1 = "<div id=\"add1\" style=\"display:block\"><table>"
    cnt1 = 0
    cnt2 = 0
    cnt3 = 0
    s2 = "<div id=\"add2\" style=\"display:none\"><table>"
    s3 = "<div id=\"add3\" style=\"display:none\"><table>"
    ordered_values.compact.each do |value|
    
      cus_name = value.custom_field.name
      regex1 = "add1_(\w*)"
      regex2 = "add2_(\w*)"
      ...
      if cus_name.match(regex1)      
        ...
        if cnt1 % 2==1
          s1 << "<tr>"
          s1 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n"
        else
          s1 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n"
          s1 << "</tr>"
        end
        
      elsif cus_name.match(regex2)
        if issue.status.name !="未启动" 
          cnt2 = cnt2+1
          if cnt2 % 2==1
            s2 << "<tr>"
            s2 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n"
          else
            s2 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n" 
            s2 << "</tr>"
          end          
        ...
        if cnt3 % 2==1
          s3 << "<tr>"
          s3 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n"
        else
          s3 << "\t<th>#{ h(value.custom_field.name) }:</th><td>#{ h(show_value(value)) }</td>\n" 
          s3 << "</tr>"
        end
  ...
  def render_custom_fields_rows_adapt(issue)
    values = issue.visible_custom_field_values
    return if values.empty?
    ordered_values = []
    half = (values.size / 2.0).ceil
  ...
  def details_to_strings(details, no_html=false, options={})
    ...
    details.each do |detail|
      ...
      if (detail.property=='attr' && (detail.prop_key=='create_project' || detail.prop_key=='destroy_project'|| detail.prop_key=='destroy_issue' || detail.prop_key=='create_issue'))
        next
      else
